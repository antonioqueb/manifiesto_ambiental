...
class ManifiestoAmbiental(models.Model):
    ...
    numero_manifiesto = fields.Char(
        ...
        default=lambda self: self.env['ir.sequence'].next_by_code('manifiesto.salida') or 'New',  # ← AGREGAR
    ...
    def _save_version_to_history_with_data(self, data_estructurados):
        ...
        try:
            ...
            version_data = {
                ...
                'data_file': data_encoded,  # ← CAMBIO: usar data_file en lugar de pdf_file
                'data_filename': f"Manifiesto_{self.numero_manifiesto}_v{self.version}_datos.txt",  # ← CAMBIO: usar data_filename
                ...
                # NUEVO: Guardar documento físico si existe
...
"""
    ...
    def _save_version_to_history(self, pdf_data):
        ...
        try:
            version_data = {
                'manifiesto_id': self.original_manifiesto_id.id,
                'version_number': self.version,
                'pdf_file': pdf_data,  # ← MANTENER: PDFs van en pdf_file
                'pdf_filename': f"Manifiesto_{self.numero_manifiesto}_v{self.version}.pdf",  # ← MANTENER: PDFs van en pdf_filename
                'creation_date': fields.Datetime.now(),
                'created_by': self.env.user.id,
                'state_at_creation': self.state,
                'change_reason': self.change_reason or f"Versión {self.version} guardada antes de remanifestación",
    ...
    def _prepare_version_data(self, next_version):
        ...
        new_vals.update({
            ...
            'change_reason': '',
            'state': 'draft',
            # NUEVO: Limpiar documento físico en nueva versión para que se suba uno nuevo
...
class ManifiestoAmbientalVersion(models.Model):
    ...
    # Archivos PDF - YA NO OBLIGATORIOS
    pdf_file = fields.Binary(
        string='Archivo PDF',
        help='PDF de esta versión del manifiesto (si existe)'
    )
    ...
    # NUEVOS CAMPOS PARA DATOS ESTRUCTURADOS (TXT)
    ...
    # NUEVOS CAMPOS PARA DOCUMENTO FÍSICO HISTÓRICO
    ...
    # Datos de referencia para vista rápida (opcional)
    generador_nombre = fields.Char(
        string='Generador',
    ...
    def action_download_file(self):
        """
        Acción para descargar el archivo de esta versión (PDF o datos)
        """
        # Intentar primero con PDF
        ...
        # Si no hay PDF, intentar con datos
        elif self.data_file and self.data_filename:
            return {
                'type': 'ir.actions.act_url',
                'url': f'/web/content/{self._name}/{self.id}/data_file/{self.data_filename}?download=true',
                'target': 'self',
            }
        else:
    ...
    def action_view_file(self):
        """
        Acción para visualizar el archivo de esta versión (PDF o datos)
        """
        # Intentar primero con PDF
        if self.pdf_file and self.pdf_filename:
            return {
                'type': 'ir.actions.act_url',
                'url': f'/web/content/{self._name}/{self.id}/pdf_file/{self.pdf_filename}',
                'target': 'new',
            }
        # Si no hay PDF, intentar con datos
        elif self.data_file and self.data_filename:
            return {
                'type': 'ir.actions.act_url',
                'url': f'/web/content/{self._name}/{self.id}/data_file/{self.data_filename}',
                'target': 'new',
            }
        else:
            raise UserError("No hay archivo disponible para esta versión.")
                ...
                <t t-call="web.external_layout">
                    <div class="page">
                        ...
                        <style>
                            ...
                            .header, 
                            .o_company_document_layout .header, 
                            .company_address, 
                            .o_company_address,
                            .external_layout .header,
                            .address,
                            .o_report_layout_standard .header,
                            .o_report_layout_boxed .header,
                            .o_report_layout_clean .header {
                            ...
                            /* ASEGURAR ESPACIO DESPUÉS DE LA CABECERA */
                            .article {
                        ...
                        <table>
                            <tr>
                                <td colspan="5" class="labelcell">
                                    4. Nombre o razón social del generador: <span t-field="doc.generador_nombre"/>
                                </td>
                            </tr>
                            <tr>
                                <td class="subcell" style="width:15%;">Domicilio</td>
                                <td class="subcell" style="width:25%;">Código postal: <span t-field="doc.generador_codigo_postal"/></td>
                                <td class="subcell" style="width:40%;">Calle: <span t-field="doc.generador_calle"/></td>
                                <td class="subcell" style="width:10%;">Núm. Ext.: <span t-field="doc.generador_num_ext"/></td>
                                <td class="subcell" style="width:10%;">Núm. Int.: <span t-field="doc.generador_num_int"/></td>
                            </tr>
                            <tr>
                                <td class="subcell" style="width:33.3%;">
                                    Colonia: <span t-field="doc.generador_colonia"/>
                                </td>
                                <td class="subcell" style="width:33.3%;">
                                    Municipio o Delegación: <span t-field="doc.generador_municipio"/>
                                </td>
                                <td class="subcell" style="width:33.4%;">
                                    Estado: <span t-field="doc.generador_estado"/>
                                </td>
                            </tr>
                            <tr>
                                <td class="subcell" style="width:50%;">
                                    Teléfono: <span t-field="doc.generador_telefono"/>
                                </td>
                                <td colspan="4" class="subcell" style="width:50%;">
                                    Correo electrónico: <span t-field="doc.generador_email"/>
                                </td>
                            </tr>
                        </table>

                        <!-- 5. IDENTIFICACIÓN DE LOS RESIDUOS CON MÍNIMO 10 FILAS -->
                        <table>
                        <tr>
                        ...
                        <!-- Mostrar residuos existentes -->
                        <t t-foreach="doc.residuo_ids" t-as="residuo">
                            <tr>
                                <td><span t-field="residuo.nombre_residuo"/></td>
                                <!-- Clasificación CRETIB - Marcar X según corresponda -->
                                <td class="center-text">
                                    <span t-if="residuo.clasificacion_corrosivo">X</span>
                                </td>
                                <td class="center-text">
                                    <span t-if="residuo.clasificacion_reactivo">X</span>
                                </td>
                                <td class="center-text">
                                    <span t-if="residuo.clasificacion_explosivo">X</span>
                                </td>
                                <td class="center-text">
                                    <span t-if="residuo.clasificacion_toxico">X</span>
                                </td>
                                <td class="center-text">
                                    <span t-if="residuo.clasificacion_inflamable">X</span>
                                </td>
                                <td class="center-text">
                                    <span t-if="residuo.clasificacion_biologico">X</span>
                                </td>
                                <td class="center-text">
                                    <!-- M - Mutagénico: no está en tus opciones, se deja vacío -->
                                </td>
                                <!-- Envase -->
                                <td><span t-field="residuo.envase_tipo"/></td>
                                <td><span t-field="residuo.envase_capacidad"/></td>
                                <!-- Cantidad - ACTUALIZADO para mostrar siempre kg -->
                        ...
                        <table>
                            ...
                            <tr>
                                <td>
                                    <strong>Colonia:</strong> <span t-field="doc.transportista_colonia"/>
                                </td>
                                <td>
                                    <strong>Municipio o Delegación:</strong> <span t-field="doc.transportista_municipio"/>
                                </td>
                                <td colspan="2">
                                    <strong>Estado:</strong> <span t-field="doc.transportista_estado"/>
                                </td>
                            </tr>
                            <tr>
                                <td>
                                    <strong>Teléfono:</strong> <span t-field="doc.transportista_telefono"/>
                                </td>
                                <td colspan="3">
                                    <strong>Correo electrónico:</strong> <span t-field="doc.transportista_email"/>
                                </td>
                            </tr>
                        </table>
                        ...
                        <table>
                            ...
                            <tr>
                                <td>
                                    <strong>Colonia:</strong> <span t-field="doc.destinatario_colonia"/>
                                </td>
                                <td>
                                    <strong>Municipio o Delegación:</strong> <span t-field="doc.destinatario_municipio"/>
                                </td>
                                <td colspan="2">
                                    <strong>Estado:</strong> <span t-field="doc.destinatario_estado"/>
                                </td>
                            </tr>
                            <tr>
                                <td>
                                    <strong>Teléfono:</strong> <span t-field="doc.destinatario_telefono"/>
                                </td>
                                <td colspan="3">
                                    <strong>Correo electrónico:</strong> <span t-field="doc.destinatario_email"/>
                                </td>
                            </tr>
                        </table>
